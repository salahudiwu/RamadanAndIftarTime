...
# Dropdown zum Ändern der Sprache
st.session_state.lang = st.selectbox(
    "Sprache auswählen / Select Language / اختر اللغة",
    options=["de","en","fr","es","tr","id","ur,ru"],
    index=["de","en","fr","es","tr","id","ur,ru"].index(st.session_state.lang)
)
lang = st.session_state.lang

# --- 2. Texte die übersetzt werden sollen ---
ui_texts = {
...
# --- 3. Übersetzung automatisch, außer arabische Inhalte ---
translator = Translator()

def translate_ui(text, dest):
    # Prüfe, ob Text arabische Zeichen enthält
    if re.search(r'[\u0600-\u06FF]', text):
        return text  # arabisch unverändert
    else:
        return translator.translate(text, dest=dest).text

translated_texts = {k: translate_ui(v, lang) for k, v in ui_texts.items()}

t = translated_texts

# --- 4. Nutzung der übersetzten Texte ---
st.title(t["title"])
